...
In a second experiment within the same study, Smoli\'{c} et al. investigated whether few-shot prompting --- providing the LLM with example code/grade pairs before asking it to score new submissions --- could improve grading reliability. The results show that few-shot prompting can better align LLM output with grading criteria, though the effect is limited in scope \cite{Smolic2024MIPRO}. Yuen et al. \cite{yuen2024prompting} corroborate the general point from a different angle: across four prompting strategies for class-level code generation, richer prompt context consistently improved LLM output quality, confirming that what goes into the prompt is a primary driver of LLM behavior on code tasks. Few-shot prompts and call-site context are complementary but orthogonal changes: few-shot prompts give the model evaluative context, while call-site context gives it structural context.
